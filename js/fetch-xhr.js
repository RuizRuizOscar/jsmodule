...
const deleteUser = (idUser) => {
    fetch(`https://koders1gpython-default-rtdb.firebaseio.com/oscar/users/${idUser}.json`, {
        method: 'DELETE'
    })
    .then(response => {
        return response.json()
...
const readUsers = () => {
    ...
    fetch(`https://koders1gpython-default-rtdb.firebaseio.com/oscar/users/.json`, options )
    .then(response => {
        return response.json()
    })
    .then((users) => {  
...
let btn_guardar = document.querySelector('.button_guardar')
if(btn_guardar){
    btn_guardar.addEventListener('click', event =>{
        ...
        if(name === '' || lastName === '' || url_photo === ''){
            ...
            createUserFetch(userToAdd)
        }
    })
}
// const crearUsuario = () => {
//     document.querySelector(".type_text_name").forEach((text)=> {
//         btn.addEventListener('click', function(event) {
//             console.log(event)
//             console.log(event.target)
//             console.log(event.target.classList)
